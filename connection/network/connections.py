...
class ConnectionManager:
    ...
    def authenticate(self, connection: tuple[bytes, any]):
        # Unpack the credentials from the connection data.
        ...
        # If no account is found, return a status code.
    ...
    def new_connection(self, account: dict, connection: tuple[bytes, any]):
        account_id = ObjectId(account['networks'][0]['id'])
        network = self.filter_object('networks', {'_id': account_id})
        new_connection = dict(
    ...
    def assign_address(self, network: dict, account: dict):
        ...
        for address_id in range(network['max_address']):
            ...
            # If the IP address is not in use, assign it to the
            # account.
